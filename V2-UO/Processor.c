...
int Processor_FetchInstruction()
{
	...
	{
		ComputerSystem_DebugMessage(95, HARDWARE, Clock_GetTime());
	}
	else
	{
		ComputerSystem_DebugMessage(94, HARDWARE, Clock_GetTime());
	}
...
void Processor_DecodeAndExecuteInstruction()
{
	...
	case OS_INST:
		...
		if (Processor_PSW_BitState(EXECUTION_MODE_BIT) == 1)
		{
			ComputerSystem_DebugMessage(69, HARDWARE, InstructionNames[operationCode], operand1, operand2, registerPC_CPU, registerAccumulator_CPU, registerPSW_CPU,
										Processor_ShowPSW());
